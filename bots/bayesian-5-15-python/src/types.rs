...
pub struct BayesianPrediction {
    ...
    pub confidence: f64,
    pub edge: f64,
    pub signals: Vec<BayesianSignal>,
    pub strike_price: Option<f64>,
    pub current_price: Option<f64>,
}

impl BayesianPrediction {
    pub fn should_trade(&self, min_trade_edge: f64) -> bool {
        self.edge >= min_trade_edge
    }

    pub fn direction(&self) -> TradeSide {
        if (self.p_up - self.p_down).abs() < 0.03 {
            if self.p_down >= self.p_up {
                TradeSide::Down
            } else {
                TradeSide::Up
            }
        } else if self.p_up > self.p_down {
            TradeSide::Up
        } else {
            TradeSide::Down
        }
